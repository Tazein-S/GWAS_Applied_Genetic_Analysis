...
##Assumes PLINK pgen output file with #CHROM POS and P columns; plots all p-values (need to filter prior to running)
...
plotscan<-function(pvalue,chrom,physloc,plotname,filename,type="bmp",plotchar=20){
  ...
  if(nchrom==1)y$newloc<-y$physloc
...
if(type == "tiff")
{
  ...
  plot(y$newloc,-log10(y$pvalue),pch=plotchar,xaxt="n",
  ...
  if(nchrom>1)abline(v=chromlist$chrend,lty=2)
  abline(h=-log10(5e-8),lty=3)
  if(nchrom>1)axis(1,at=chromlist$midpt,labels=chromlist$chr,cex.axis=1.5)
  if(nchrom==1)axis(1,labels=unique(chrom),at=median(y$newloc),cex.axis=1.5)
  dev.off()
}

##Rscript --vanilla gwaplot_pgen.R filename plottitle plotfile 
##Assumes PLINK output file with #CHROM POS and P columns; plots all p-values (need to filter prior to running)
...
plottitle<-args[2]
plotfile<-args[3]
yy<-read.table(infile,header=T,as.is=T,comment.char="")
if(colnames(yy)[1]=="CHR"){
	colnames(yy)[1]<-"X.CHROM"
	colnames(yy)[3]<-"POS"}
yy<-subset(yy,X.CHROM>0)
plotscan(yy$P,yy$X.CHROM,yy$POS,plottitle,plotfile,type="jpeg",plotchar=20)
